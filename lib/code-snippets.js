...
const HTTPSnippet = require('../httpsnippet/src/index');
...
codesnippets.toCode = function (json, settings, callback) {
    ...
    let out = [];
    let input = Hoek.clone(json);
    let options = Hoek.clone(settings.snippets);
    delete options.lanugages;

    // Add in appendParameters into each endpoint parameters set
    if(Array.isArray(Hoek.reach(settings, 'snippets.appendParameters'))){
        const pathKeys = Object.keys(input.paths)
        pathKeys.forEach((pathKey) => {
            const path = input.paths[pathKey];
            const methodKeys = Object.keys(path)
            methodKeys.forEach((methodKey) => {
                if(!path[methodKey].parameters){
                    path[methodKey].parameters = [];
                }
                path[methodKey].parameters = path[methodKey].parameters.concat(settings.snippets.appendParameters);
            })
        })
    }

    this.toRequests(input, options, function (err, requestArr) {
